...
TabelaHash* criarTabelaHash(int tamanho) {
    ...
    if (tabela != NULL) {
        ...
        tabela->listas = (Nodo**)malloc(tamanho * sizeof(Nodo*));
...
int funcaoHash(TabelaHash* tabela, int value) {
    return value % tabela->tamanho;
}

int somarAscii(const char* string) {
...
void inserir(TabelaHash* tabela, Pokemon poke) {
    int indice = funcaoHash(tabela, somarAscii(poke.name));

    Nodo* novo = criarNodo(poke);
    if (novo != NULL) {
        novo->proximo = tabela->listas[indice];
        tabela->listas[indice] = novo;
    }
}

int buscar(TabelaHash* tabela, char* name) {
    int indice = funcaoHash(tabela, somarAscii(name));
    Nodo* atual = tabela->listas[indice];
    
    bool continua = (atual != NULL) ? true : false;
    int achou = -1;
    while (continua) {
        if(atual == NULL){
            continua = false;
        }
        else if (strcmp(atual->pokemon.name, name)== 0) {
            continua = false;
            achou = indice;
        }
        else {
...
{
    FILE *arquivo = fopen("matrícula_hashIndireta.txt", "w");
...
int main()
{
    ...
    TabelaHash* tabela = criarTabelaHash(21);
    ...
    while (strcmp(entrada, "FIM") != 0)
    {
        ...
        if (strcmp(entrada, "FIM") != 0)
        {
            ...
            inserir(tabela, Buscar_Pokemon_ID(id_input, poke, QTD_POKEMONS));
        }
    }
    ...
    while (strcmp(entrada, "FIM") != 0)
    {
        ...
        {
            printf("=> %s: ", entrada);
            int achou = buscar(tabela, entrada);
            if(achou != -1){
