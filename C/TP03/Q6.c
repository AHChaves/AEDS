...
int size_lista(Pilha_Pokemon *l){
    return l->size;
}

void insert_P(Pilha_Pokemon *l, Pokemon elemento){    
    l->ultimo->prox = new_celula(elemento);
    l->ultimo = l->ultimo->prox;
    l->size++;
}

Pokemon remove_at(Pilha_Pokemon *l, int pos){
...
Pokemon remove_P(Pilha_Pokemon *l){
    return remove_at(l, l->size-1);
}

void print_lista(Pilha_Pokemon *l){
    Celula *i;
    int j;
...
void delete_lista(Pilha_Pokemon *l){
    while(l->size > 0)
        remove_at(l,0);
    free(l->primeiro);
}
...
int main()
{
    ...
    Pilha_Pokemon pokeSelect = new_lista();
    ...
    while (strcmp(entrada, "FIM") != 0)
    {
        ...
        if (strcmp(entrada, "FIM") != 0)
        {
            ...
            insert_P(&pokeSelect, Buscar_Pokemon_ID(id_input, poke, QTD_POKEMONS));
        }
    }
    ...
    for (int i = 0; i < n_operacoes; i++)
    {
        ...
        if (strcmp(operacao, "I") == 0)
        {
            int id;
            scanf("%d", &id);
            insert_P(&pokeSelect, Buscar_Pokemon_ID(id, poke, QTD_POKEMONS));
        }
        else if (strcmp(operacao, "R") == 0)
        {
            Pokemon p = remove_P(&pokeSelect);
            printf("(R) %s\n", p.name);
        }
    }
