...
typedef struct LP
{
    Pokemon array[TAM_FILA_CIRC]; // Elementos da pilha
    int n;                       // Quantidade de array.
    int inicio;
    int fim;
} Fila_Circ_Pokemon;

void start_LP(Fila_Circ_Pokemon *l)
{
    l->n = 0;
    l->fim = 0;
    l->inicio = 0;
...
Pokemon remover_FC(Fila_Circ_Pokemon* l)
{
    int i;
    Pokemon resp;
    ...
    {
        resp = l->array[l->inicio];
        l->inicio = (l->inicio + 1) % TAM_FILA_CIRC;
        l->n--;
    }
...
void inserir_FC(Pokemon x, Fila_Circ_Pokemon* l)
{
    if (l->n >= TAM_FILA_CIRC)
        remover_FC(l);
...
int main()
{
    ...
    while (strcmp(entrada, "FIM") != 0)
    {
        ...
        if (strcmp(entrada, "FIM") != 0)
        {
            ...
            inserir_FC(Buscar_Pokemon_ID(id_input, poke, QTD_POKEMONS), &pokeSelect);
        }
    }
    ...
    for (int i = 0; i < n_operacoes; i++)
    {
        ...
        if (strcmp(operacao, "I") == 0)
        {
            int id;
            scanf("%d", &id);
            inserir_FC(Buscar_Pokemon_ID(id, poke, QTD_POKEMONS), &pokeSelect);
        }
        else if (strcmp(operacao, "R") == 0)
        {
            Pokemon p = remover_FC(&pokeSelect);
            printf("(R) %s\n", p.name);
        }
    }
