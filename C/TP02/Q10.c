...
void quicksortRec(Pokemon *poke, int esq, int dir) {
    int i = esq, j = dir;
    Pokemon pivo = poke[(dir + esq) / 2];
    while (i <= j) {
        // Ordena primeiro pelo height, em caso de empate ordena pelo name
    ...
    if (esq < j) quicksortRec(poke, esq, j);
    if (i < dir) quicksortRec(poke, i, dir);
}
